...
void LinuxSocket::setUdp(const int port)
{
...
bool LinuxSocket::sendUdp ( const std::string s ) const
{
...
bool LinuxSocket::sendUdp ( void* data, int length ) const
{
    int status = ::sendto ( m_sock, data, length, 0, (struct sockaddr*) &m_addr, sizeof(m_addr) );
    if ( status == -1 )
    {
        return false;
    }
