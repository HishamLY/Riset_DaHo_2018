...
CameraCalibration* httpd::cam_calibration;
CameraCalibration* httpd::cam_finder;
...
ColorFinder* httpd::field_finder;
...
void httpd::command(int fd, char *parameter) {
  ...
  else
  {
      ...
      else if(strcmp(section, "green") == 0) finder = field_finder;
      else if(strcmp(section, "white") == 0) finder = white_finder;
      else if(strcmp(section, "cameracalib") == 0) cam_finder = cam_calibration;
  }
...
{
    int res = -1;
    double m_GoalFBStep = 30;
    double m_GoalRLTurn = 30;
    double X_step_counter = 0;
    double Y_step_counter = 0;
    double A_step_counter = 0;
    double m_FBStep = 0;
    double m_RLTurn = 0;
...
//=======CAMERA CALIBRATION ======
    ...
    /*Camera Calibration*/
    case IN_CMD_LOAD_CAMCALIB:
        if(cam_finder== NULL) return;
        cam_finder->LoadINISettings(ini);

        strcpy(res_str, "RELOAD");
        break;
    case IN_CMD_SAVE_CAMCALIB:
        if(finder == NULL) return;
        cam_finder->SaveINISettings(ini);
        strcpy(res_str, "SAVE");
        break;
    case IN_CMD_CAMROLL_SET:
        if(cam_finder ==NULL) return;
        cam_finder->cameraRollCorrection = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_CAMROLL_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->cameraRollCorrection += value;
        if(cam_finder->cameraRollCorrection > 1.57) cam_finder->cameraRollCorrection = 1.57;
        sprintf(res_str, "%f", cam_finder->cameraRollCorrection);
        break;
    case IN_CMD_CAMROLL_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->cameraRollCorrection -= value;
        if(cam_finder->cameraRollCorrection < -1.57) cam_finder->cameraRollCorrection = -1.57;
        sprintf(res_str, "%f", cam_finder->cameraRollCorrection);
        break;
    case IN_CMD_CAMTILT_SET:
        if(cam_finder ==NULL) return;
        cam_finder->cameraTiltCorrection = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_CAMTILT_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->cameraTiltCorrection += value;
        if(cam_finder->cameraTiltCorrection > 1.57) cam_finder->cameraTiltCorrection = 1.57;
        sprintf(res_str, "%f", cam_finder->cameraTiltCorrection);
        break;
    case IN_CMD_CAMTILT_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->cameraTiltCorrection -= value;
        if(cam_finder->cameraTiltCorrection < -1.57) cam_finder->cameraTiltCorrection = -1.57;
        sprintf(res_str, "%f", cam_finder->cameraTiltCorrection);
        break;
    case IN_CMD_CAMPAN_SET:
        if(cam_finder ==NULL) return;
        cam_finder->cameraPanCorrection = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_CAMPAN_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->cameraPanCorrection += value;
        if(cam_finder->cameraPanCorrection > 1.57) cam_finder->cameraPanCorrection = 1.57;
        sprintf(res_str, "%f", cam_finder->cameraPanCorrection);
        break;
    case IN_CMD_CAMPAN_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->cameraPanCorrection -= value;
        if(cam_finder->cameraPanCorrection < -1.57) cam_finder->cameraPanCorrection = -1.57;
        sprintf(res_str, "%f", cam_finder->cameraPanCorrection);
        break;
    case IN_CMD_BODYTILT_SET:
        if(cam_finder ==NULL) return;
        cam_finder->bodyTiltCorrection = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_BODYTILT_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTiltCorrection  += value;
        if(cam_finder->bodyTiltCorrection > 1.57) cam_finder->bodyTiltCorrection = 1.57;
        sprintf(res_str, "%f", cam_finder->bodyTiltCorrection );
        break;
    case IN_CMD_BODYTILT_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTiltCorrection   -= value;
        if(cam_finder->bodyTiltCorrection   < -1.57) cam_finder->bodyTiltCorrection  = -1.57;
        sprintf(res_str, "%f", cam_finder->bodyTiltCorrection  );
        break;
    case IN_CMD_BODYROLL_SET:
        if(cam_finder ==NULL) return;
        cam_finder->bodyRollCorrection = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_BODYROLL_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyRollCorrection+= value;
        if(cam_finder->bodyRollCorrection  > 1.57) cam_finder->bodyRollCorrection = 1.57;
        sprintf(res_str, "%f", cam_finder->bodyRollCorrection  );
        break;
    case IN_CMD_BODYROLL_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyRollCorrection-= value;
        if(cam_finder->bodyRollCorrection < -1.57) cam_finder->bodyRollCorrection = -1.57;
        sprintf(res_str, "%f", cam_finder->bodyRollCorrection);
        break;
    case IN_CMD_BODYTRANSX_SET:
        if(cam_finder ==NULL) return;
        cam_finder->bodyTranslationCorrection.x = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_BODYTRANSX_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTranslationCorrection.x += value;
        if(cam_finder->bodyTranslationCorrection.x > 100 ) cam_finder->bodyTranslationCorrection.x= 100 ;
        sprintf(res_str, "%f", cam_finder->bodyTranslationCorrection.x);
        break;
    case IN_CMD_BODYTRANSX_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTranslationCorrection.x -= value;
        if(cam_finder->bodyTranslationCorrection.x < -100) cam_finder->bodyTranslationCorrection.x = -100;
        sprintf(res_str, "%f", cam_finder->bodyTranslationCorrection.x);
        break;
    case IN_CMD_BODYTRANSY_SET:
        if(cam_finder ==NULL) return;
        cam_finder->bodyTranslationCorrection.y = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_BODYTRANSY_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTranslationCorrection.y += value;
        if(cam_finder->bodyTranslationCorrection.y > 100 ) cam_finder->bodyTranslationCorrection.y= 100 ;
        sprintf(res_str, "%f", cam_finder->bodyTranslationCorrection.y);
        break;
    case IN_CMD_BODYTRANSY_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTranslationCorrection.y -= value;
        if(cam_finder->bodyTranslationCorrection.y < -100) cam_finder->bodyTranslationCorrection.y = -100;
        sprintf(res_str, "%f", cam_finder->bodyTranslationCorrection.y);
        break;
    case IN_CMD_BODYTRANSZ_SET:
        if(cam_finder ==NULL) return;
        cam_finder->bodyTranslationCorrection.z = value;
        sprintf(res_str, "%f", value);
        break;
    case IN_CMD_BODYTRANSZ_PLUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTranslationCorrection.z += value;
        if(cam_finder->bodyTranslationCorrection.z > 100 ) cam_finder->bodyTranslationCorrection.z = 100 ;
        sprintf(res_str, "%f", cam_finder->bodyTranslationCorrection.z);
        break;
    case IN_CMD_BODYTRANSZ_MINUS:
        if(cam_finder == NULL) return;
        cam_finder->bodyTranslationCorrection.z -= value;
        if(cam_finder->bodyTranslationCorrection.z < -100) cam_finder->bodyTranslationCorrection.z = -100;
        sprintf(res_str, "%f", cam_finder->bodyTranslationCorrection.z);
...
//=======WALK TUNER ======
    ...
    case IN_CMD_WALK_B_HIP_ROLL_GAIN:
        ...
        break;
	case IN_CMD_WALK_B_HIP_PITCH_GAIN:
        Walking::GetInstance()->BALANCE_HIP_PITCH_GAIN += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->BALANCE_HIP_PITCH_GAIN);
        break;
...
//edited for odometri tuning
    case IN_CMD_WALK_X_ODOMETRI_GAIN:
        Walking::GetInstance()->X_MOVE_SCALE += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->X_MOVE_SCALE);
        break;
    case IN_CMD_WALK_Y_ODOMETRI_GAIN:
        Walking::GetInstance()->Y_MOVE_SCALE += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->Y_MOVE_SCALE );
        break;
    case IN_CMD_WALK_A_ODOMETRI_GAIN:
        Walking::GetInstance()->A_MOVE_SCALE  += value;
        sprintf(res_str, "%.2f", Walking::GetInstance()->A_MOVE_SCALE );
        break;
    case IN_CMD_WALK_X_TUNNING:
        if(value == 1)
        {
            strcpy(res_str, "ON");
            while (Walking::GetInstance()->X_MOVE_COUNTER < 1000)
            {
                if(Walking::GetInstance()->IsRunning() == false)
                {
                    Walking::GetInstance()->X_MOVE_COUNTER  = 0;
                    m_FBStep = 0;
                ...
                Walking::GetInstance()->X_MOVE_COUNTER  +=  Walking::GetInstance()->Get_X_Moved();
            }
        ...
        break;
    case IN_CMD_WALK_Y_TUNNING:
        if(value == 1)
        {
            strcpy(res_str, "ON");

            while(Walking::GetInstance()->Y_MOVE_COUNTER < 500)
            {
                if(Walking::GetInstance()->IsRunning() == false)
                {
                    Walking::GetInstance()->Y_MOVE_COUNTER  = 0;
                    m_FBStep = 0;
                    Walking::GetInstance()->Y_MOVE_AMPLITUDE = m_FBStep;
                    Walking::GetInstance()->Start();
                }
                else
                {
                ...
                Walking::GetInstance()->Y_MOVE_COUNTER +=  Walking::GetInstance()->Get_Y_Moved();
            }
            if(Walking::GetInstance()->IsRunning() == true)
                Walking::GetInstance()->Stop();
            Walking::GetInstance()->Y_MOVE_COUNTER = 0;
        ...
        break;
    case IN_CMD_WALK_A_TUNNING:
        if(value == 1)
        {
            //Walking::GetInstance()->Start();
            strcpy(res_str, "ON");

	    while(Walking::GetInstance()->A_MOVE_COUNTER < 3.14)
            {
                if(Walking::GetInstance()->IsRunning() == false)
                {
                    Walking::GetInstance()->A_MOVE_COUNTER  = 0;
                    m_RLTurn = 0;
		    ...
		    if(m_RLTurn < m_GoalRLTurn)
                ...
                Walking::GetInstance()->A_MOVE_COUNTER +=  Walking::GetInstance()->Get_A_Moved();
            }
            if(Walking::GetInstance()->IsRunning() == true)
                Walking::GetInstance()->Stop();
            Walking::GetInstance()->A_MOVE_COUNTER = 0;
    ...
    //edited for controlling the robot manually
    case IN_CMD_WALK_MOVE_FORWARD:
        if(Walking::GetInstance()->IsRunning() == true)
             Walking::GetInstance()->X_MOVE_AMPLITUDE = 30;
        else
             Walking::GetInstance()->X_MOVE_AMPLITUDE = 0;
        break;
    case IN_CMD_WALK_MOVE_BACKWARD:
        if(Walking::GetInstance()->IsRunning() == true)
             Walking::GetInstance()->X_MOVE_AMPLITUDE = -30;
        else
             Walking::GetInstance()->X_MOVE_AMPLITUDE = 0;
        break;
    case IN_CMD_WALK_MOVE_RIGHT:
        if(Walking::GetInstance()->IsRunning() == true)
             Walking::GetInstance()->Y_MOVE_AMPLITUDE = -30;
        else
             Walking::GetInstance()->Y_MOVE_AMPLITUDE = 0;
        break;
    case IN_CMD_WALK_MOVE_LEFT:
        if(Walking::GetInstance()->IsRunning() == true)
             Walking::GetInstance()->Y_MOVE_AMPLITUDE = 30;
        else
             Walking::GetInstance()->Y_MOVE_AMPLITUDE = 0;
        break;
    case IN_CMD_WALK_TURN_RIGHT:
        if(Walking::GetInstance()->IsRunning() == true)
             Walking::GetInstance()->A_MOVE_AMPLITUDE = -30;
        else
             Walking::GetInstance()->A_MOVE_AMPLITUDE = 0;
        break;
    case IN_CMD_WALK_TURN_LEFT:
        if(Walking::GetInstance()->IsRunning() == true)
             Walking::GetInstance()->A_MOVE_AMPLITUDE = 30;
        else
             Walking::GetInstance()->A_MOVE_AMPLITUDE = 0;
        break;
    case IN_CMD_WALK_START_STOP:
	if(Walking::GetInstance()->IsRunning() == false){
        ...
        break;
    case IN_CMD_WALK_PAN_PLUS:
        Head::GetInstance()->MoveByAngleOffset(5,0);
        break;
    case IN_CMD_WALK_PAN_MINUS:
        Head::GetInstance()->MoveByAngleOffset(-5,0);
        break;
    case IN_CMD_WALK_TILT_MINUS:
        Head::GetInstance()->MoveByAngleOffset(0,-5);
        break;
    case IN_CMD_WALK_TILT_PLUS:
        Head::GetInstance()->MoveByAngleOffset(0,5);
        break;
    case IN_CMD_WALK_RESET:
        Head::GetInstance()->MoveToHome();
        break;
    case IN_CMD_WALK_KICK_LEFT:
        if((Action::GetInstance()->IsRunning() == 0) && (Walking::GetInstance()->IsRunning() == false))
            {
    ...
    case IN_CMD_WALK_KICK_RIGHT:
        ...
        break;
    case IN_CMD_GUI_CHANGEVIEW:
//        if (Localization::GetInstance()->ReadCameraOrMap == 0 )
//            Localization::GetInstance()->ReadCameraOrMap = 1;
//        else
//           Localization::GetInstance()->ReadCameraOrMap = 0;
        break;
    default:
        res = -1;
    }
