...
int LinuxCamera::Initialize640(int deviceIndex)
{
    struct stat st;
    char devName[15] = {0, };
    ...
    fbuffer = new FrameBuffer(Camera::WIDTH, Camera::HEIGHT);
    printf("ImageSize: %d\n", fbuffer->m_YUVFrame->m_ImageSize);
...
void LinuxCamera::LoadINISettings()
{
    int value = -2;
    CameraSettings newset = GetCameraSettings();
...
int LinuxCamera::ReadFrame()
{
    ...
    for(int i = 0; i < buffers[buf.index].length; i++)
...
{
    //Busy waiting sampai beres switched
...
void LinuxCamera::SwitchResolution()
{
    struct v4l2_format fmt;
    CLEAR(fmt);
