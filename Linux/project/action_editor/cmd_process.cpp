...
//##################### nulis kondisi motor servo ##############//
		...
		if(step->position[id] & Action::INVALID_BIT_MASK)
			...
			printf("%.4d", step->position[id]); //berarti posisi servo sekarang ada di step->position[id]
	}
...
void HelpCmd()
{
	...
	printf(" conv               Convert ID 1 - 6 and 19 - 20 to 10 bit position\n");
	printf(" inv                Convert ID 1 - 6 and 19 - 20 to 12 bit position\n");
	printf(" mirror		    Mirroring servo position\n");
	printf("\n");
...
void PlayCmd(CM730 *cm730, Robot::LinuxMotionTimer *timer)
{
	...
	ReadStep(cm730); // baca posisi servo sekarang
	DrawStep(7); //ditulis di kolom 7
}
...
// tambahan untuk konversi file 12 bit menjadi 10 bit pada servo MX->AX
// Gunakan Hanya untuk konversi bit 12 ke 10 bit pada servo ax atau rx 
// *sesuaikan id mana saja yang akan diubah
void ConvCmd()
{
	for(int i=0; i<Page.header.stepnum; i++)
	{
		for (int id=1 ;id <= 6; id++)
			{
				if (Page.step[i].position[id] < 343)
					Page.step[i].position[id]=0;
				else if (Page.step[i].position[id] >3753)
					Page.step[i].position[id]=1023;
				else
					Page.step[i].position[id] = (Page.step[i].position[id] - 341) * 0.3;
			}
...
// tambahan untuk konversi file 10 bit menjadi 12 bit pada servo AX->MX
// Gunakan Hanya untuk konversi bit 12 ke 10 bit pada servo ax atau rx 
// *sesuaikan id mana saja yang akan diubah
void InvCmd()
{
	for(int i=0; i<Page.header.stepnum; i++)
	{
		for (int id=1 ;id <= 6; id++)
			{
				Page.step[i].position[id] = Page.step[i].position[id]/0.3 + 341;	
			}
/*		for (int id=19 ;id <= 20; id++)
			{
				Page.step[i].position[id] = Page.step[i].position[id]/0.3 + 341;
...
//tambahan untuk Mencerminkan nilai motor servo dari gerakan kanan menjadi kiri atau sebaliknya (untuk servo dengan resolusi 4096 bit)
void MirrorCmd()
{
	int temp_position[18];
	for(int i=0; i<Page.header.stepnum; i++)
	{
		for (int id=JointData::ID_R_SHOULDER_PITCH ;id <= JointData::ID_L_ANKLE_ROLL; id++)
			{
				temp_position[id] = Page.step[i].position[id];
			}
		for (int id=JointData::ID_R_SHOULDER_PITCH ;id <= JointData::ID_L_ANKLE_ROLL; id++)
			{
				if (id % 2 == 1)
					Page.step[i].position[id]=  MX28::GetMirrorValue(temp_position[id+1]);
				else 
					Page.step[i].position[id]=  MX28::GetMirrorValue(temp_position[id-1]);
			}
		Page.step[i].position[JointData::ID_HEAD_PAN] = MX28::GetMirrorValue(Page.step[i].position[JointData::ID_HEAD_PAN]);
		DrawStep(i);
	}
	Action::GetInstance()->SavePage(indexPage, &Page);
}		//   */
