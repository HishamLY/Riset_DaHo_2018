...
int main(void)
{
	...
	finder->FilteringImage(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame);

	double NewTiltAngle = Head::GetInstance()->GetTiltAngle();
        int yHorizon = 120 +  (NewTiltAngle - 50) * ((double)Camera::HEIGHT / Camera::VIEW_V_ANGLE);

        if(yHorizon < 0)
        	yHorizon = 2;
        pos = finder->GetPositionMoment(yHorizon);
