...
void DrawScreen()
{
    ...
    printf("X_ODOMETRI_SCALE          \n"); // 8
    printf("Y_ODOMETRI_SCALE          \n"); // 9
    printf("A_ODOMETRI_SCALE          \n"); // 0
    printf("X_TUNNING                 \n"); // 1
    printf("Y_TUNNING                 \n"); // 2
    printf("Z_TUNNING                 \n"); // 3
    ClearCmd(); // 4
    ...
    //edited for odometri tuning
    GoToCursor(PARAM_COL, X_MOVE_SCALE_ROW);
	printf("%.2f    ", Walking::GetInstance()->X_MOVE_SCALE);

	GoToCursor(PARAM_COL, Y_MOVE_SCALE_ROW);
	printf("%.2f    ", Walking::GetInstance()->Y_MOVE_SCALE);

	GoToCursor(PARAM_COL, A_MOVE_SCALE_ROW);
	printf("%.2f    ", Walking::GetInstance()->A_MOVE_SCALE);

    GoToCursor(PARAM_COL, X_TUNNING_ROW);
	printf("%.2f    ", Walking::GetInstance()->X_MOVE_COUNTER );

	GoToCursor(PARAM_COL, Y_TUNNING_ROW);
	printf("%.2f    ", Walking::GetInstance()->Y_MOVE_COUNTER );

    GoToCursor(PARAM_COL, A_TUNNING_ROW);
	printf("%.2f    ", Walking::GetInstance()->A_MOVE_COUNTER );
...
void IncreaseValue(bool large)
{
    ...
    //odometri tuning
    case X_MOVE_SCALE_ROW:
        if(large == true)
			Walking::GetInstance()->X_MOVE_SCALE += 0.1;
		else
			Walking::GetInstance()->X_MOVE_SCALE += 0.01;
		printf("%.2f    ", Walking::GetInstance()->X_MOVE_SCALE);

    case Y_MOVE_SCALE_ROW:
        if(large == true)
			Walking::GetInstance()->Y_MOVE_SCALE += 0.1;
		else
			Walking::GetInstance()->Y_MOVE_SCALE += 0.01;
		printf("%.2f    ", Walking::GetInstance()->Y_MOVE_SCALE);

    case A_MOVE_SCALE_ROW:
        if(large == true)
			Walking::GetInstance()->A_MOVE_SCALE += 0.1;
		else
			Walking::GetInstance()->A_MOVE_SCALE += 0.01;
		printf("%.2f    ", Walking::GetInstance()->A_MOVE_SCALE);
