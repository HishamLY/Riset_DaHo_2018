...
/* ============ Imu Calibration ================== */
  Imu::GetInstance()->CalibrateIMU();
    ...
    Action::GetInstance()->Start(9);//tadinya 15 duduk
...
//			sp = 144; //60;186
...

    {
		...
		field_finder->FilteringImageErotionDilation(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame); //field finder baru untuk menghilangkan noise
		...
		//==========================Tuning Walking===============================
		//Walking::GetInstance()->HIP_PITCH_OFFSET = (follower.m_GoalRLStep > 10 || follower.m_GoalFBStep < 0 || follower.m_GoalRLTurn > 10)?23.0:21.0;
		//=======================================================
...
//    teamCom->teamData[3].x = 12;
	...
	if(StatusCheck::m_cur_mode == PENALTY_KICK)
        {
		field_finder->FilteringImageErotionDilation(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame); // field finder baru 
		ball_finder->FilteringImage(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame);
		Vector2<int> ppfball;
...
*/
							...
							|| Behavior::GetInstance()->get_state() == LOCALIZE_GOALPOST)
      {
	    	...
	    	yellow_finder->FilteringImage(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame);
        blue_finder->FilteringImage(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame);
				field_finder->FilteringImageErotionDilation(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame); //baru untuk menghilangkan noise
...
*/
		...
		else if(Behavior::GetInstance()->get_state() == GOBACK_STATE)
		{
	                yellow_finder->FilteringImage(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame);
	                blue_finder->FilteringImage(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame);
			field_finder->FilteringImageErotionDilation(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame); // menghilangkan noise
			ball_finder->FilteringImage(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame);
		        for(int i = 1; i < rgb_output->m_NumberOfPixels; i++)//luqman
        		{
			if(ball_finder->m_result->m_ImageData[i] == 1)
		...
		//mode ini dia terus mengirimkan jarak dia dengan bola
			...
			|| Behavior::GetInstance()->get_state() == PASS_BALL_STATE)
	   	{
			...
			int n = sprintf(sendDataTeamCom,"%s#%d#%lf#%lf#%lf", teamCom->DataHeader.c_str(), teamCom->PlayerStatus, Head::GetInstance()->GetTiltAngle(), 1.0, 1.0);
//			int n = sprintf(sendDataTeamCom,"%s#%d#%lf#%d#%d", teamCom->DataHeader.c_str(), teamCom->PlayerStatus, Head::GetInstance()->GetTiltAngle(), 0, 0);
                	if(n > 0)
                	{
                	        if(teamCom->sender->send(sendDataTeamCom, n)); //printf("SUCCESS SENDING UDP\n");
                	}
