...
void ColorFinder::Detect(Image * img, int r_min, int r_max, std::vector<Point2D> * center)
{
  int x, y, xMax, yMax;
  int count, thresh, r_circle;
  int maxAcc = -1;
  bool find = false;
  ...
  for (int r = r_min; r <= r_max; r++)
  {
    ...
    for (int i = 0; i < img->m_NumberOfPixels; i++)
    {
      if (img->m_ImageData[i * img->m_PixelSize + 0] == 255)
      {
        ...
        x = i % Camera::WIDTH;
        y = i / Camera::WIDTH;
        Accum_circle(accumulator, x, y, r, &count);
      }
    }
    // thresh = 5 * r;
    // thresh = 10 * r;
  ...
  // find = false;
  // drawCircle(img, xMax, yMax, r_circle);
  // Draw::Circle(img, Point2D(xMax,yMax), 3, ColorRGB(255,255,0));
  // *center = Point2D(xMax,yMax);
}

void ColorFinder::Accum_circle(int * accum, int i, int j, int rad, int * count)
{
  int f = 1 - rad;
  int dx = 1;
  int dy = -2 * rad;
  ...
  Accum_pixel(accum, i, j + rad, count);
  Accum_pixel(accum, i, j - rad, count);
  Accum_pixel(accum, i + rad, j, count);
  Accum_pixel(accum, i - rad, j, count);
  ...
  while(x < y)
  {
    ...
    Accum_pixel(accum, i + x, j + y, count);
    Accum_pixel(accum, i - x, j + y, count);
    Accum_pixel(accum, i + x, j - y, count);
    Accum_pixel(accum, i - x, j - y, count);
    Accum_pixel(accum, i + y, j + x, count);
    Accum_pixel(accum, i - y, j + x, count);
    Accum_pixel(accum, i + y, j - x, count);
    Accum_pixel(accum, i - y, j - x, count);
  }
...
void ColorFinder::Accum_pixel(int * accum, int x, int y, int * count) {
