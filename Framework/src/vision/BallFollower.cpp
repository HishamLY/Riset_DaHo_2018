...
BallFollower::BallFollower()
{
	...
	CountMaxTurnBall = 0; //dipake turning ball
	...
	m_UnitRLTurn = 3.0;	// 3.0
	m_UnitRLStep = 3;
	m_UnitZMove = 3; //inc foot height //Andreas
	...
	m_RLTurn = 0;
	m_RLStep = 0;
	m_Zmove = 25; //init foot height //Andreas
...
void BallFollower::ProcessSearchingBall()
{
		...
		Behavior::GetInstance()->get_state() == APROACHING_BALL)
	{
		...
		else
		{
			...
			m_GoalRLTurn = 0;
			m_GoalRLStep = 0;
			Walking::GetInstance()->X_MOVE_AMPLITUDE = 0;//diubah
			Walking::GetInstance()->A_MOVE_AMPLITUDE = 0;
...
void BallFollower::ProcessAproachingBall(Point2D ball_pos, Vector2<int> ppfBall, bool EdgeField)
{
	...
	if(m_Zmove < m_GoalZMove)
		m_Zmove += m_UnitZMove;
	else if(m_FBStep >= m_GoalZMove)
		m_Zmove -= m_UnitZMove;
	Walking::GetInstance()->Z_MOVE_AMPLITUDE = m_Zmove;
...
void BallFollower::ProcessAproachingBall2(Point2D ball_pos, double panpost, double CompassError, double CompassPost)
{
	...
	else //BOLA SUDAH KETEMU
	{
		...
		printf("BOLA BELUM LURUS - ");
			if( tilt > 25) //bola masih jauh di depan //Al 0
			{
				printf("JAUH - ");
				...
				else
				{
					...
					m_GoalRLTurn = pan*0.4; //putar pelan karena bola sudut robot-bola-gawang sudah kecil
					m_GoalFBStep = 10;
				}
			}
...
void BallFollower::ProcessToKick(Point2D ball_pos, double PanPost, double CompassError, double CompassPost)
{
	if(ball_pos.X == -1.0 || ball_pos.Y == -1.0)        // CEK JIKA BOLA TIDAK KETEMU
	{
		...
		{
			Walking::GetInstance()->X_MOVE_AMPLITUDE = 0; //bahaya nilai x terlalu kecil
			Walking::GetInstance()->A_MOVE_AMPLITUDE = 0;
			Walking::GetInstance()->Y_MOVE_AMPLITUDE = 0;
	...
	else	// CEK JIKA BOLA KETEMU
	{
		...
		else if (tilt <= 20 && abs(pan) <= 18)
		{
			if(CompassError >= 13 && CompassError < 40)
			{
				m_GoalFBStep = -3;
				m_RLStep = 23;
				m_GoalRLTurn = -5;
				printf("LURUSKAN 1111111111111111111\n");
			}
			else if(CompassError < -13 && CompassError > -40)
			{
				m_GoalFBStep = -4;
				m_RLStep = -30;
				m_GoalRLTurn = 5;
				printf("LURUSKAN 22222222222222222222\n");
			}
			else if (CompassError >= 40)
			{
				m_GoalFBStep = -5;
            m_RLStep = 30;
            m_GoalRLTurn = -5;
            printf("LURUSKAN 33333333333333333333\n");
			}
			else if (CompassError <= -40)
			{
				m_GoalFBStep = -5;
            m_RLStep = -33;
            m_GoalRLTurn = 5;
            printf("LURUSKAN 44444444444444444444\n");
			}
			else
			{
				m_GoalRLStep = 0;
				m_GoalRLTurn = 0;
				m_GoalFBStep = 24;
				printf("MAJUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU SIKAATTTTT BROOOOOOOOOOOOOOOOO\n");
			}
		}
		else
		{
			Behavior::GetInstance()->set_normal_state();
			printf("BOLA JAUUUUHH\n");
		}
