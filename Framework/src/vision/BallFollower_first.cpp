...
{
bool geser_kiri = false;
bool geser_kanan = false;
	if(DEBUG_PRINT == true)
		fprintf(stderr, "\r                                                                               \r");
...
/*					if(pan > 0 && pan <= 10)
	...
	else
	{
		...
		{
			/*if(geser_kiri) {Walking::GetInstance()->X_MOVE_AMPLITUDE = 0; Walking::GetInstance()->Y_MOVE_AMPLITUDE = m_RLTurn;}
			else if(geser_kanan) {Walking::GetInstance()->X_MOVE_AMPLITUDE = 0; Walking::GetInstance()->Y_MOVE_AMPLITUDE = m_RLTurn;}
			{*/
