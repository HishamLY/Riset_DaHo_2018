	...
	else //BOLA SUDAH KETEMU
	{
		...
		if(fabs(CompassError + pan) >= fabs(temp) + 5) //robot belum lurus ke arah bola-gawang
		{
			...
			if( tilt > 15) //bola masih jauh di depan //Al 0
			{
				printf("JAUH - ");
				//m_GoalFBStep = 4;
				if(fabs(pan) > 30) //bola ada jauh di samping
				{
...
/*
		...
		printf("\t\t\t\t\t\t\t\t\t\t TILT : %lf", tilt);
		printf("\t\t\t\t\t\t\t\t\t\t PAN : %lf\n", pan);
		printf("\t\t\t\t\t\t\tt\t\t\t PAN POST + COMPAS : %lf\n", CompassError+PanPost);
		
		if((tilt >= -20) && (abs(pan) <= 18)) // BOLA MASIH JAUH
		{
...
*/
		...
		else if (tilt <= 10) 
		{
			...
			{
				m_GoalFBStep = 8;
				m_GoalRLTurn = pan*0.45;//tadinya 0.5
				printf("BOLA DISAMPING\n");
			}
			else // pan <= 19 
			{
				if(CompassError+PanPost*0.8 >= 15)//tadinya 0,9 dan 10
				{
					m_GoalFBStep = 6; // 
					//m_RLTurn = 0;
					m_GoalRLTurn = 21;//tadinya 25 
					printf("LURUSKAN\n");
				}
				else if(CompassError+PanPost*0.8 < -15)
				{
					m_GoalFBStep = 6; // 
					//m_RLTurn = 0;
					m_GoalRLStep = -21;
