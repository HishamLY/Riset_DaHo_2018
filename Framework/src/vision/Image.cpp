...
Image::~Image()
{
		if(Behavior::GetInstance()->Bingung == FlagSwitched)
			delete[] m_ImageData;//ini jangan pernah di komen, biar RAMnya gak penuh...
    m_ImageData = 0;//ini jugaaaaa.....
    FlagSwitched = Behavior::GetInstance()->Bingung;
}


void Image::switch_resolution(int width, int height, int pixelsize)
{
        m_Width=width;
        m_Height=height;
        m_PixelSize=pixelsize;
