...
void LinePerceptor::update(LinePercept& linePercept, LineSpots& linespots, Image* image, CameraMatrix& thecameramatrix)
{
  theLineSpots = &linespots;
  ...
  Geometry::calculatePointOnField((int)pp1.x, (int)pp1.y, *theCameraMatrix, ppf);
  Geometry::calculatePointOnField((int)pp2.x, (int)pp2.y, *theCameraMatrix, ppf2);
  //Draw::Line(img, Point2D(pp1.x, pp1.y), Point2D(pp2.x, pp2.y),ColorRGB(ColorClasses::black)); //module:LinePerceptor:naoMeter
  //Draw::Line(img, Point2D(pp1.x, pp1.y-5), Point2D(pp1.x, pp1.y+5), ColorRGB(ColorClasses::black)); //module:LinePerceptor:naoMeter
  //Draw::Line(img, Point2D(pp2.x, pp2.y-5), Point2D(pp2.x, pp2.y+5), ColorRGB(ColorClasses::black)); //module:LinePerceptor:naoMeter

  lineSegs.clear(); // a list inside lineperceptor
  linePercept.clear();  //from linePecept (function parameter)
  banSectors.clear(); //list inside lineperceptor

  linePercept.InitPercept();

  createLineSegments(linePercept.singleSegs);
  createLines(linePercept.lines, linePercept.singleSegs);
  analyzeSingleSegments(linePercept.singleSegs, linePercept.circle, linePercept.lines);
  analyzeLines(linePercept.lines, linePercept.intersections, linePercept.circle, linePercept.singleSegs);

  linePercept.drawOnImage(image, *theCameraMatrix, theFieldDimensions, parameters.circleBiggerThanSpecified, theImageCoordinateSystem);
}
...
void LinePerceptor::createBanSectors()
{
  for(std::vector<LineSpots::NonLineSpot>::const_iterator spot = theLineSpots->nonLineSpots.begin(); spot != theLineSpots->nonLineSpots.end(); spot++)
    {
      const Vector2<> p1_cor = theImageCoordinateSystem.toCorrected(spot->p1), //it had a value
                               p2_cor = theImageCoordinateSystem.toCorrected(spot->p2); //it had too

      Vector2<> pf1, pf2;
      if(!Geometry::calculatePointOnFieldHacked((int)p1_cor.x, (int)p1_cor.y, *theCameraMatrix, pf1))
        {
      ...
      if(!Geometry::calculatePointOnFieldHacked((int)p2_cor.x, (int)p2_cor.y, *theCameraMatrix, pf2))
        {
          pf2 = pf1 * 100;
          Draw::Cross(img, Point2D(((spot->p1+spot->p2)/2).x, ((spot->p1+spot->p2)/2).y), 4, ColorRGB(ColorClasses::orange)); //LineSegmentsImg
          //continue;
        }

      //Draw::Arrow(fld, Point2D(pf1.x, pf1.y), Point2D(pf2.x, pf2.y), ColorRGB(ColorClasses::orange)); //draw on Field
      //ARROW("module:LinePerceptor:NonLineSegments", , , 30, Drawings::ps_solid, ColorClasses::orange);
  ...
  for(std::list<BanSector>::iterator s = banSectors.begin(); s != banSectors.end(); s++)
    {
      ...
      Vector2<int> p1((int)(cosf(s->alphaLeft) * s->start), (int)(sinf(s->alphaLeft) * s->start));
      Vector2<int> p2((int)(cosf(s->alphaRight) * s->start), (int)(sinf(s->alphaRight) * s->start));
      Vector2<int> p3((int)(cosf(s->alphaLeft) * s->end), (int)(sinf(s->alphaLeft) * s->end));
      Vector2<int> p4((int)(cosf(s->alphaRight) * s->end), (int)(sinf(s->alphaRight) * s->end));
  ...
  for(std::list<BanSector>::iterator s = banSectors.begin(); s != banSectors.end(); s = nexts)
    {
      ...
      Geometry::calculatePointInImage(Vector2<int>((int)p1cor.x, (int)p1cor.y), *theCameraMatrix, p1Img);
      Geometry::calculatePointInImage(Vector2<int>((int)p2cor.x, (int)p2cor.y), *theCameraMatrix, p2Img);
      Geometry::calculatePointInImage(Vector2<int>((int)p3cor.x, (int)p3cor.y), *theCameraMatrix, p3Img);
      Geometry::calculatePointInImage(Vector2<int>((int)p4cor.x, (int)p4cor.y), *theCameraMatrix, p4Img);

      Draw::Line(img, Point2D( p1Img.x, p1Img.y), Point2D(p2Img.x, p2Img.y), ColorRGB(ColorClasses::red));
      Draw::Line(img, Point2D( p1Img.x, p1Img.y), Point2D(p3Img.x, p3Img.y), ColorRGB(ColorClasses::red));
      Draw::Line(img, Point2D( p2Img.x, p2Img.y), Point2D(p4Img.x, p4Img.y), ColorRGB(ColorClasses::red));
      Draw::Line(img, Point2D( p3Img.x, p3Img.y), Point2D(p4Img.x, p4Img.y), ColorRGB(ColorClasses::red));
      ...
      if(spot->alpha_len/spot->alpha_len2 > parameters.minWidthRatio)
        {
          ...
          Vector2<int> pf1, pf2;
          if(!Geometry::calculatePointOnField((int)p1_cor.x, (int)p1_cor.y, *theCameraMatrix, pf1))
            {
                Draw::Cross(img, Point2D(((spot->p1+spot->p2)/2).x, ((spot->p1+spot->p2)/2).y), 4, ColorRGB(ColorClasses::orange));
              //CROSS("module:LinePerceptor:LineSegmentsImg", ((spot->p1+spot->p2)/2).x, ((spot->p1+spot->p2)/2).y,4, 2, Drawings::ps_solid, ColorClasses::orange);
              continue;
            }
          if(!Geometry::calculatePointOnField((int)p2_cor.x, (int)p2_cor.y, *theCameraMatrix, pf2))
            {
                Draw::Cross(img, Point2D(((spot->p1+spot->p2)/2).x, ((spot->p1+spot->p2)/2).y), 4, ColorRGB(ColorClasses::orange));
              //CROSS("module:LinePerceptor:LineSegmentsImg", ((spot->p1+spot->p2)/2).x, ((spot->p1+spot->p2)/2).y,4, 2, Drawings::ps_solid, ColorClasses::orange);
              continue;
          ...
          if(pf1 == pf2)
            {
              Draw::Cross(img, Point2D(((spot->p1+spot->p2)/2).x, ((spot->p1+spot->p2)/2).y), 4, ColorRGB(ColorClasses::robotBlue));
              continue;
            }

          if(pf1 * pf1 > sqr(parameters.maxLineDistance) || pf2 * pf2 > sqr(parameters.maxLineDistance))
            {
                //drawonfield harusnya
                //Draw::Cross(img, Point2D(((pf1+pf2)/2).x, ((pf1+pf2)/2).y), 20, ColorRGB(ColorClasses::yellow));
          ...
          if(fabs(fabs(spot->alpha) - pi_2) < nonLineParams.maxAlphaDiff && spot->alpha_len / spot->alpha_len2 > nonLineParams.minWidthRatio && (pf1 - pf2).squareAbs() > sqr(nonLineParams.minLineLength))
            {
            Draw::Arrow(img, Point2D(spot->p1.x, spot->p1.y), Point2D(spot->p2.x, spot->p2.y), ColorRGB(ColorClasses::yellow));
           // Draw::Arrow(img, Point2D(pf1.x, pf1.y), Point2D( pf2.x, pf2.y), ColorRGB(ColorClasses::yellow));
          ...
          while(s.alpha > pi)
            s.alpha -= pi;
          const float d1 = s.p1.x * cosf(s.alpha) + s.p1.y * sinf(s.alpha),
                           d2 = s.p2.x * cosf(s.alpha) + s.p2.y * sinf(s.alpha);
          s.d = (d1 + d2) / 2.0f;
          lineSegs.push_back(s);
          //printf("%f,%f--->%f,%f \n",spot->p1.x, spot->p1.y,spot->p2.x, spot->p2.y);

          Draw::Arrow(img, Point2D(spot->p1.x, spot->p1.y), Point2D(spot->p2.x, spot->p2.y), ColorRGB(ColorClasses::blue));
        }
    }
      ...
      for(std::list<LinePercept::Line>::iterator line = lines.begin(); line != lines.end(); line++)
        {
          ...
          if(fabs(seg->p1.x * cosf(line->alpha) + seg->p1.y * sinf(line->alpha) - line->d) < parameters.maxLineSingleSegDist &&
              fabs(seg->p2.x * cosf(line->alpha) + seg->p2.y * sinf(line->alpha) - line->d) < parameters.maxLineSingleSegDist)
            {
  ...
  for(std::list<LinePercept::Line>::iterator l1 = lines.begin(); l1 != lines.end(); l1++)
    {
      ...
      if(hardcover/(float)(l1->first - l1->last).abs() < parameters.minHardcover)
        toDelete.push_back(l1);
    }
  ...
  for(std::list<LinePercept::Line>::iterator l1 = lines.begin(); l1 != lines.end(); l1++)
    {
      ...
      for(l2++; l2 != lines.end(); l2++)
        {
          ...
          while(alphaDiff > pi_2)
            alphaDiff -= pi;
          //printf("alphadiff , minIntersectionAlphadiff = %f , %f \n",fabs(alphaDiff), parameters.minIntersectionAlphaDiff);
      ...
      //printf("k = %i\n", k);
          ...
          //equate and disolve two hess ash normal form, then comes out of it
          const float zaehler = l1->d - (l2->d * cosf(l1->alpha)/cosf(l2->alpha)),
                                nenner = sinf(l1->alpha) - (sinf(l2->alpha)*cosf(l1->alpha)/cosf(l2->alpha));
          const float y_s = zaehler/nenner,
                            x_s = (l1->d - y_s * sinf(l1->alpha))/cosf(l1->alpha);
          ...
          if(y_s == y_s && x_s == x_s)//intersection exists -> not paralel || ident
            {
              ...
              //printf("distol1 = %f , distol2 = %f , param =%i\n", distToL1, distToL2, parameters.minTToEnd);

               if((distToL1 < -parameters.minTToEnd && distToL2 < parameters.maxTFromEnd) ||
             ...
             else if(distToL1 < -parameters.minTToEnd && distToL2 < -parameters.minTToEnd)
                {
                  //ARROW("module:LinePerceptor:Intersections", x_s, y_s, l1->last.x, l1->last.y, 5, Drawings::ps_solid, ColorClasses::yellow);
                  //ARROW("module:LinePerceptor:Intersections", x_s, y_s, l2->last.x, l2->last.y, 5, Drawings::ps_solid, ColorClasses::yellow);
                  //this is a X
  ...
  for(std::list<LinePercept::CircleSpot>::iterator spot_iter = circleSpots.begin(); spot_iter != circleSpots.end(); spot_iter++)
    {
      ...
      if((int)supporters.size() >= circleParams.minSupporters)
        {
          printf("suporter size %i \n", supporters.size());
          center /= supporters.size();
          /*
          //bagian ini masih segmentation fault dan kadang glibc detected
...
void LinePerceptor::analyzeSingleSegments(std::list<LinePercept::LineSegment>& singleSegs, LinePercept::CircleSpot& circle, std::list<LinePercept::Line>& lines)
{
  ...
  std::list<std::list<LinePercept::LineSegment>::iterator> toDelete;
  ...
  for(std::list<LinePercept::CircleSpot>::iterator spot_iter = circleSpots.begin(); spot_iter != circleSpots.end(); spot_iter++)
    ...
    for(std::list<std::list<LinePercept::LineSegment>::iterator>::iterator d = toDelete.begin(); d != toDelete.end(); ++d)
  {
    std::list<std::list<LinePercept::LineSegment>::iterator>::iterator i = d;
